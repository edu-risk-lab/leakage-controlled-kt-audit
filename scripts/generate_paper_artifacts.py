...
def _write_cold_start_by_stratum_tex(path: Path, *, model: str = "simplekt", fold: int = 0) -> None:
    ...
    lines = [
        ...
        r"(Section~\ref{sec:cold-start}). The $n$ column counts validation+test held-out interactions whose KC ",
        r"falls in the stratum train-frequency bin; \emph{out of range} marks held-out KCs ",
        r"outside the four canonical bins. Stratum mass varies by dataset split; under default bins ",
        r"Synthetic C2/C5 route all held-out interactions into the hot stratum ($>500$ train-fold hits per \KC{}).}",
