...
M4_SEED42 = ROOT / "results" / "q1" / "m4_q0.95_k5_K5000_tau0.1" / "baseline_results.csv"
M4_SEED17 = ROOT / "results" / "q1" / "m4_q0.95_k5_K5000_tau0.1_seed17" / "baseline_results.csv"
...
def _arm_auc(path: Path, construction: str) -> float:
    df = pd.read_csv(path)
    return float(df.loc[df["graph_construction"] == construction, "auc"].iloc[0])


def measured_xes_sigma(seed42: Path, seed17: Path) -> dict | None:
    """Two-run XES3G5M floor: same graph and split, different experiment seeds."""
    if not seed42.exists() or not seed17.exists():
        return None
    to42, fl42 = _arm_auc(seed42, "train_only"), _arm_auc(seed42, "full_log")
    to17, fl17 = _arm_auc(seed17, "train_only"), _arm_auc(seed17, "full_log")
    to_gap = abs(to17 - to42)
    fl_gap = abs(fl17 - fl42)
    return {
        "to_gap": to_gap,
        "fl_gap": fl_gap,
        "sigma": max(to_gap, fl_gap),
        "delta_seed42": fl42 - to42,
        "delta_seed17": fl17 - to17,
    }
...
def write_macros(
    ...
    transfer_strict: dict | None = None,
    ...
    if transfer is not None:
        lines += [
            ...
            rf"\newcommand{{\NoiseFloorPrimaryHigh}}"
            rf"{{\ensuremath{{{_sci(float(transfer['sigma_target_est']))}}}}}",
        ]
    if transfer_strict is not None:
        lines += [
            rf"\newcommand{{\NoiseFloorPrimaryLow}}"
            rf"{{\ensuremath{{{_sci(float(transfer_strict['sigma_target_est']))}}}}}",
        ]
    if measured is not None:
        lines += [
            rf"\newcommand{{\NoiseFloorPrimaryMeasTO}}"
            rf"{{\ensuremath{{{_sci(float(measured['to_gap']))}}}}}",
            rf"\newcommand{{\NoiseFloorPrimaryMeas}}"
            rf"{{\ensuremath{{{_sci(float(measured['sigma']))}}}}}",
        ]
    lines.append("")
...
def main() -> int:
    ...
    measured = measured_xes_sigma(M4_SEED42, M4_SEED17)
    write_macros(
        summary,
        null,
        args.out_macros,
        transfer=transfers[0][1],
        transfer_strict=transfers[1][1],
        measured=measured,
    )
    ...
    if measured is not None:
        print(
            "\nMeasured XES3G5M floor (published-default cell, seed 17 vs 42, "
            "same split and graph):"
            f"  train-only gap {measured['to_gap']:.3e}"
            f"  full-log gap {measured['fl_gap']:.3e}"
            f"  -> sigma {measured['sigma']:.3e}"
        )
