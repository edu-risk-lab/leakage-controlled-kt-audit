...
def write_cold_start_summary_tex(df: pd.DataFrame, path: Path) -> None:
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Cold-start KC diagnostic summary. Column $n$ counts fold~0 test "
        r"interactions per stratum for \textit{simpleKT}; \textit{simpleKT} AUC and "
...
def write_ddr_downstream_anova_tex(ddr_df: pd.DataFrame, path: Path) -> None:
    ...
    lines = [
        ...
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Exploratory ANOVA on DGEKT downstream AUC under graph perturbation "
        r"(ASSISTments and XES3G5M; three folds per operator/strength; Table~S20). Dependent variable: "
        r"test AUC after retraining on the perturbed graph. Partial $\eta^2$ as above. "
        r"This analysis supports the null predictive-sensitivity finding in "
        r"Section~\ref{sec:exp-ddr-downstream}; it does not establish backbone dominance.}",
